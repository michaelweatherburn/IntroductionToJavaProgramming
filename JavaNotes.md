1)
Specific Import
when the exact class you're trying to import is defined.
Example:
java.util.Scanner //This will import the Scanner class into the current file's context
Wildcard Import
When there is no exact class to import
Example:
java.util.*   //This will import all classes inside the java.lang package

There is no performance difference between Specific Import and Wildcard Import, since both imports simply tell the compiler where the files are

2) Types of errors, syntax, semantic and logical errors
Syntax Errors
The errors that are present in the grammar of your source code. Every programming language has rules, and if source code is not created according to those rules, syntax errors follow. These errors are caught by the compiler. (The compiler is the program that checks for Syntax Errors, and proceeds to convert source code to bytecode in Java, if there are no syntax errors).

Semantic Errors (Also called Runtime Errors)
The errors 
...
19) To represent Unicode characters, we can use the Unicode character convention:
...
stringObject.trim(), removes all whitespace characters on both ends of String, returns new String
...
These methods are analogous to indexOf, but check from end to beginning (in the reverse direction). If no match is found, all of these methods return -1.
...
Format Flags provide additional information as to how to format the string. For example:
a) , (Comma, applicable to numbers only) (%,10d) Add commas to numbers that are larger than 1,000 according to the international notation system (commas after every three digits).
b) - (Alignment) (%-10s) - Change the alignment to left-aligned. By default, all strings and numbers are right-aligned.
c) 0 (Zero) (%010d, applicable to numbers only) - By default, leading spaces are not shown if the maximum field width for the formatted output is greater than the number being output.
...
a) Entry-controlled Loops, aka pre-test loops (for, while)
b) Exit-controlled loops aka post-test loops (do..while)
...
34) If the loop-control variable that you're using in a for-loop will not be used anywhere else, it is a good practice to declare it inside the initialization section of the for-loop. This will cause the variable to only have scope inside the braces of that for-loop (and, of course, the conditional-check expression/update expression as well).

It is allowed to have:

a) Multiple initialization/declaration statements
...
If multiple initializations/update-expressions exists, they need to be separated by commas. Also, the variables declared in the initialization section have to be of the same datatype.
...
a) You can skip initialization if the loop counter variable has already been initialized before the body of the loop.
...
In a for loop, the action-after-each-iteration following the lines of code that were skipped is executed first. continue ONLY skips over the REST OF THE LOOP BODY in the current iteration - it doesn't impact anything in the loop flowchart.

In all three types of loops, using a break statement COMPLETELY exits the loop.
...
43) When a method is called, the values of the actual parameters (arguments) are used to initialize the formal parameters (parameters). A copy of each actual parameter is created and assigned to the corresponding formal parameter. This is called Pass By Value.
...
Note that for method overloading to work, the method signature must be unique. Modifiers and returntypes don't form part of the method signature, so having the same method signature but different returntypes/modifiers would result in a compilation error.
...
49) Scope of a variable is the part of the program in which the variable can be referenced.
...
50) Two variables can have the same name, as long as their scopes do NOT intersect. Thus, two variables declared in separate blocks of code can have the same name, as the separation of the blocks causes the scope of the variables to not intersect.
...
51) Method Abstraction and Stepwise Refinement
...
52) It is not possible to do declarations of multiple types in a for loop.

for (int i = 1, j = 3, k; j < k; i++) {
  // valid for loop
}

for (int i = 1, double j = 4.0...) // invalid initial-action for the for loop

53) Arrays are a fixed-size sequential collection of elements of the same datatype.
...
This is called array variable initialization - it initializes the "array variable" i.e. a "reference variable" used to store "references to arrays" with a "reference". The actual array elements are not stored in the object (non-static/instance variable)/class (static variable)/activation record of the method within the runtime stack (method local variable/parameter).
...
Primitive Variable Default Values:
...
null is a special literal (one of the three literal keywords, the other two being true and false)
It is a literal used to denote the absence of reference. (It is similar to void, where void is a keyword which is used to denote the absence of a returntype in methods).

54) We use array indices (unique numbers used to reference each element within the array) along with the array reference variable to access/manipulate individual elements of the array. The index convention is as follows:
...
first element of the array has the index 0
...
55) There is another syntax used for declaring arrays if you already know their contents. It is called the array initializer:
...
58) arraycopy method in Java is used to copy certain elements from one array to another.
...
numberOfElements refers to the number of elements that are to be copied from the source array (starting at sourceArrayStartingIndex) to the destination array (starging at destinationArrayStartingIndex)
...
59) Garbage Collection - when the program is running, there can be arrays/objects in the heap that have no reference variables pointing to them in the program. When that occurs, JVM silently frees up the memory allocated to those objects so that they can be used by other objects (objects that have no references anymore cannot be used by the program anymore, so they are just wasting space in the heap). This process of deallocation of memory in the heap is called Garbage Collection.

60) System.nanoTime() is used to return the number of nanoseconds that have passed since some arbitrary reference point in time. This is useful when one wishes the know the nanosecond level difference between two events that have a very small time difference between them.

61) Passing Arrays to Methods
...
63) Arrays are objects in Java. That is why an array has a fixed static variable called length that can be accessed by using the dot-operator syntax.
Yes, even arrays of primitive variables are objects in Java.

64) Returning Arrays from Method - When a method returns an array, the reference variable of that array is returned. All the regular pass-by-value rules concerning reference datatypes apply. The syntax of the method header:

modifiers returnDatatype[] methodName(formalParameterDatatype formalParameter1, formalParameterDatatype formalParameter2...)

65) Variable-Length Argument Lists
...
Rules: 
a) Only one variable-length argument list is allowed in the method definition header.
b) All the inputs provided as actual parameters (arguments) must have datatypes that are the same as/can be implicitly casted to the type of the formal parameter.
c) There can be other arguments besides the variable-length argument list. If that is the case, the varargs must be the last actual parameter(argument) in the method definition.
